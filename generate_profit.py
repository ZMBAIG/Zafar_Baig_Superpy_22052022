...
def get_profit(args):
    ...
    if args.now:
        day                 = to_get_change_day(0)
        console.print(f"Total Profit/Loss on inventory stock for today, {day}, = [bold red on white]€{profit}")
    if args.yesterday:
        day                 = to_get_change_day(-1)
        print(f"Total Profit/Loss on inventory stock for yesterday, {day}, = [bold red on white]€{profit}")
    if args.date:
        print(f"Total Profit/Loss on inventory stock on {args.date[0].date()} = [bold red on white]€{profit}")
    if args.period:
        print(
            f"Total Profit/Loss on inventory stock for period between {args.period[0].date()} and {args.period[1].date()} = [bold red on white]€{profit}"
        )
